...
double State(const std::vector<int>&oper_list, const double &h)
{
...
void diagonal_update(std::vector<int>&spinsf,std::vector<int>&spinsm, std::vector<int>&spinsl, std::vector<int>&oper_list, std::uniform_real_distribution<double> &dis2,std::uniform_int_distribution<int>&dis3, const int &Ns, const int &Nb, std::mt19937 &gen, const std::vector<int>&up, const double&h)
{
	...
	for(int i = 0; i<p; i++)
	{
		...
		{
			double prob_accept = h*Ns/(h*Ns+J*Nb);
			while(1){
				int site2 =dis3(gen);
...
int main()
{
	...
	name << "RESULT_SSE_cluster_p_ground_Sx:" << "_" << int(h) <<"_" <<L;
	std::ofstream out(name.str());
	for(int j= 0; j<10; j++)
	{
		p = p*2;
		std::vector<int> oper_list(p,4);
		//std::vector<double> Magnet1(sweep);
		...
		//auto [M2_M,M2_ME] = Measure2(Magnet2, binning);
		double maxLogW = *std::max_element(logw.begin(), logw.end());
		double denom = 0.0, numer = 0.0;
		for (int i=0; i<sweep; i++)
	       	{
    			double w = std::exp(logw[i] - maxLogW);
    			denom += w;
    			numer += w * energy[i];
		}
		double EM = numer/denom;
