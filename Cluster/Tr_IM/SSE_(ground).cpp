...
double State(const std::vector<int>&oper_list, const double &h)
{
	...
	for(int i=0;i<p;i++)
	{
		...
		{
			logS = logS+std::log(2*J);
		}
	}
	return logS;
}
double Energy(const std::vector<int>&oper_list, const double &h)
{
	int p = oper_list.size();
	double E =0.0;
	int n0 = 0;
...
int main()
{
	...
	for(int j= 0; j<5; j++)
	{
		...
		for(int i=0; i<sweep; i++)
		{
			...
			energy[i] = Energy(oper_list,h)/double(L);
			diagonal_update(spinsf,spinsm, spinsl, oper_list, dis2, dis3, Ns,Nb, gen,up,h);
		...
		double denom = 0.0, numer1 = 0.0, numer2 = 0.0;
		for (int i=0; i<sweep; i++)
	       	{
		...
		double EM = numer1/denom;
		double MM = numer2/denom;
