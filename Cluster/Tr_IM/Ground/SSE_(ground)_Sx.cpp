...
double Energy(const std::vector<int>&oper_list, const double &h, const std::vector<int>&spinsm, const std::vector<int>&up)
{
	...
	E=E+std::abs(test)/double(L);
        return (E);

}

void diagonal_update(std::vector<int>&spinsf,std::vector<int>&spinsm, std::vector<int>&spinsl, std::vector<int>&oper_list, std::uniform_real_distribution<double> &dis2,std::uniform_int_distribution<int>&dis3,  const int &Nb, std::mt19937 &gen, const std::vector<int>&up, const double&h)
{
	int L = spinsl.size();
	int p = oper_list.size();
	for(int i= 0;i<L;i++)
	...
	for(int i = 0; i<p; i++)
	{
		if(oper_list[i]%3 ==2)
		{
			...
			spinsl[up[site]] = -spinsl[up[site]];
		}
		else
		{
			double prob_accept = 2*h*L/(2*h*L+J*Nb);
			while(1){
				int site2 = dis3(gen);
...
void off_diagonal_update(std::vector<int>&X_list, std::vector<int>&V_first,std::vector<int>&V_last,  std::vector<int>&oper_list, std::uniform_real_distribution<double> &dis2, std::mt19937 &gen,std::vector<int>&spinsf, std::vector<int>&spinsl)
{
	...
	for(int i=0;i<p;i++)
	{
		if(oper_list[i]%3==0||oper_list[i]%3==2)
		{
			int v0 = 4*i+n;
			int v1 = v0+1;
			int v2 = v0+2;
...
int main()
{
	...
	name << "RESULT_SSE_cluster_p_ground_Sx_:"<<int(h) << "_test" <<L;
	...
	for(int j= 0; j<1; j++)
	{
		...
		{
			diagonal_update(spinsf,spinsm, spinsl, oper_list,dis2,dis3,Nb,gen,up,h);
		...
		{
			energy[i] = Energy(oper_list,h,spinsm,up);
			diagonal_update(spinsf,spinsm, spinsl, oper_list, dis2, dis3,Nb, gen,up,h);
